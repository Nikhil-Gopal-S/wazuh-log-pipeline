...
RUN pip3 install "fastapi[standard]" uvicorn slowapi --break-system-packages


RUN apt-get update && apt-get install -y  procps curl apt-transport-https ca-certificates gnupg2 inotify-tools gettext-base gosu --no-install-recommends && apt-get clean \
&& rm -rf /var/lib/apt/lists/*
...
RUN apt-get update && if [ -n "$WAZUH_AGENT_VERSION" ]; then apt-get install -y wazuh-agent="$WAZUH_AGENT_VERSION" --no-install-recommends; else apt-get install -y wazuh-agent --no-install-recommends; fi && apt-get clean \
&& rm -rf /var/lib/apt/lists/*
